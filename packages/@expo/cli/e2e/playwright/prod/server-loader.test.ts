...
test.describe('server loader (SSR) in production', () => {
  ...
  test('loader endpoint returns JSON', async ({ page }) => {
    // Fetch loader data directly
    const response = await page.request.get(expoServe.url.href + '_expo/loaders/second');
  ...
  test('loader endpoint returns JSON with params for dynamic route', async ({ page }) => {
    const response = await page.request.get(expoServe.url.href + '_expo/loaders/posts/my-test-post');
    expect(response.status()).toBe(200);
    expect(response.headers()['content-type']).toContain('application/json');
